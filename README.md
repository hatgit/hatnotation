...
As noted above, the **exception when leading zeroes should NOT be discarded** is when the length of the string being encoded is already equal to zero when modulo 6, and thus leading zeroes are already present such as in the following string, `001111111101110110000010000001010100110110010101010010100100100000` which when encoded whould appear also as `F^=21K=LI!W` and indistinguishable from an indentical string that lacked such two leading zeroes prior to encoding, and thus the user must retain the initial length of the string prior to encoding, in order to decided whether any leading zeroes should be discarded or not when decoding back to the initial data. 
